...
namespace GameClient
{
    public partial class FormPlaying : Form
    {
        ...
        delegate void PicBoxDelegate();
        PicBoxDelegate setPicBoxEnableDelegate;
        ...
        public FormPlaying(int TableIndex, int Side, StreamWriter sw)
        {
            ...
            setPicBoxEnableDelegate = new PicBoxDelegate(setPicBoxEnable);
        ...
        private void pictureBox1_MouseDown(object sender, MouseEventArgs e)
        {
            int x = e.X / 20;
            int y = e.Y / 20;
            //SetDot(x, y, DotColor.Black);
            if (grid[x - 1, y - 1] == DotColor.None)
            {
                grid[x - 1, y - 1] = (DotColor)side;
            }
            ...
            if (!(x < 1 || x > 15 || y < 1 || y > 15))
            {
                if (grid[x - 1, y - 1] != DotColor.None)
                {
                    ...
                    service.SendToServer(string.Format(
                       "setDot,{0},{1},{2},{3},{4}", tableIndex, side, x - 1, y - 1, color));
                }
            }
        }
        /// <summary>
        ...
        public void setPicBoxEnable()
        {
            if (pictureBox1.InvokeRequired)
            {
                pictureBox1.Invoke(setPicBoxEnableDelegate);
            }
            else
            {
